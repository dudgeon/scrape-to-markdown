...
- `src/shared/template-validation.ts` — `validateTemplateStore(input)` validates JSON input against the `TemplateStore` shape. Returns `{ valid: true, templates }` or `{ valid: false, error }`. Used by the options page import flow. Pure function, no Chrome deps.
